...
- `/kit/os` is the complete rebuilt OS inventory: the shared production library plus OS-native desktop, window, menu bar, dock, widget, master-detail, split-view, command bar, and settings-sheet structures. Its five source theme moods are maintained as solid OKLCH systems with 44px controls; glass, aura, glow, raw colors, and source application governance do not travel.
...
- `lib/kit-capability-contract.json` is the shared release contract. Purple Rain, JADE, and OS must carry the same state coverage, responsive range, accessibility requirements, and production application patterns before any can claim broader capability.
- `registry.json` publishes Purple Rain under `/r/`; `registry/jade/registry.json` publishes JADE under `/r/jade/`; `registry/os/registry.json` publishes OS under `/r/os/`. Every installed piece carries its own system tokens automatically.
- `lib/purple-rain-library.json` is the counted shared inventory; `lib/os-library.json` extends it with OS-specific structures; the three registry generators keep showrooms and delivery in sync.
