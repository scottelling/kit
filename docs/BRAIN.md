...
- `lib/purple-rain-library.json` is the 142-piece everyday inventory; `lib/os-library.json` and `lib/animation-library.json` preserve the specialist source ownership; `lib/universal-library.json` is the 179-piece catalog exposed through all five systems. Specialist pieces are individually available but never pulled into a project unless requested.
