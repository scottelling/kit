...
export function ForgotPasswordForm({ children, className, ...props }: ForgotPasswordFormProps) {
  if (children !== undefined) {
    return (<section data-slot="forgot-password-form" className={cn("min-w-0 rounded-[var(--radius-card)] bg-card text-card-foreground", className)} {...props}>{children}</section>)
  }
