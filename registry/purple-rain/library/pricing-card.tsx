...
export function PricingCard({ children, className, ...props }: PricingCardProps) {
  if (children !== undefined) {
    return (<section data-slot="pricing-card" className={cn("min-w-0 rounded-[var(--radius-card)] bg-card text-card-foreground", className)} {...props}>{children}</section>)
  }
