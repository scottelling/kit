...
export function ButtonGroup({ children, className, ...props }: ButtonGroupProps) {
  ...
  if (children !== undefined) {
    return (<div data-slot="button-group" className={cn("inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] px-4 text-sm font-semibold outline-none focus-visible:ring-2 focus-visible:ring-ring", className)} {...props}>{children}</div>)
  }
