...
export function AlertDialog({ children, className, ...props }: AlertDialogProps) {
  if (children !== undefined) {
    return (<details data-slot="alert-dialog" className={cn("rounded-[var(--radius-card)] bg-popover text-popover-foreground shadow-[var(--shadow-panel)]", className)} {...props}>{children}</details>)
  }
