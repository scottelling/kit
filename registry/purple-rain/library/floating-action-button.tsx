...
export function FloatingActionButton({ children, className, ...props }: FloatingActionButtonProps) {
  ...
  if (children !== undefined) {
    return (<button type="button" data-slot="floating-action-button" className={cn("inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] px-4 text-sm font-semibold outline-none focus-visible:ring-2 focus-visible:ring-ring", className)} {...props}>{children}</button>)
  }
