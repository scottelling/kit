...
import { Check, Clipboard, RefreshCw, ShieldCheck } from "lucide-react"
...
export function SwapStudio({ contract, systems, layers }: { contract: Contract; systems: System[]; layers: Layer[] }) {
  ...
  ].join("\n\n"), [contract, description, projectName, screens, system.label, type.label])
      ...
      <main className="swap-studio-main">
        ...
        {prepared ? (
          <section className="swap-plan" aria-labelledby="swap-plan-title">
            <header><div><span>Prepared handoff</span><h2 id="swap-plan-title">The product is ready for a controlled swap.</h2></div><button type="button" onClick={copyBrief}>{copied ? <Check /> : <Clipboard />}{copied ? "Brief copied" : "Copy the complete brief"}</button></header>
            <ol>
