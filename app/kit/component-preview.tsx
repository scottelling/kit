...
export function ComponentPreview({ item, expanded = false, system = "purple-rain" }: ComponentPreviewProps) {
  ...
  const systemName = system === "jade" ? "JADE" : system === "os" ? "OS" : system === "animation" ? "Animation Studio" : system === "vanilla" ? "Vanilla" : system === "voltage" ? "Voltage" : system === "calm" ? "Calm Desktop" : "Purple Rain"
  ...
  switch (preview) {
    ...
    case "app-shell":
      sample = <div className="mini-app-shell"><aside>{system === "jade" ? "J" : system === "os" ? "OS" : system === "animation" ? "A" : system === "vanilla" ? "V" : system === "voltage" ? "⚡" : system === "calm" ? "C" : "PR"}</aside><header>Release</header><main><strong>Today</strong><span>Three things need you.</span></main></div>
      break
    case "auth":
    case "sign-in":
    case "sign-up":
    ...
    case "application-shell":
      sample = <div className="mini-application-system"><aside><b>{system === "jade" ? "J" : system === "os" ? "OS" : system === "animation" ? "A" : system === "vanilla" ? "V" : system === "voltage" ? "⚡" : system === "calm" ? "C" : "PR"}</b>{["Home", "Work", "People"].map((label, index) => <button key={label} type="button" aria-current={active === index ? "page" : undefined} onClick={() => setActive(index)}>{label}</button>)}</aside><section><header><strong>Launch room</strong><button type="button">Share</button></header><main><span>Current decision</span><strong>{["Home direction", "Release checklist", "Review team"][active]}</strong><p>The same product structure stays clear in every visual system.</p></main><footer><span>All changes saved</span><b>3 online</b></footer></section></div>
      break
    case "workspace-tree":
